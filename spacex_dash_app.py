...
{'label': 'CCAFS SLC-40', 'value': 'CCAFS SLC-40'}], value='ALL', placeholder="Select a Launch Site here",
searchable=True),
html.Br(),
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_site_df, values='class', names='class', title=f'Total Success Launches for site {entered_site}')
        return fig
# TASK 4:
...
def success_payload_scatter_chart(site_dropdown,payload_slider):
    filtered_df = spacex_df
    if payload_slider == 'ALL':
        scatter_fig = px.scatter(spacex_df,
        x='Payload Mass (kg)',
        y='class',
        ...
        y='class',
        color='Booster Version Category',
        title=f'Payload vs. Launch Outcome for {payload_slider}'
        )
        return scatter_fig
